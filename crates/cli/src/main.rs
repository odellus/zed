...
fn run() -> Result<()> {
    ...
    anyhow::ensure!(
        ...
        "Dev servers were removed in v0.157.x please upgrade to SSH remoting: https://crow-ai.dev/docs/remote-development"
    );
    ...
    impl Detect {
        pub fn detect(path: Option<&Path>) -> anyhow::Result<impl InstalledApp> {
            let path = if let Some(path) = path {
                ...
                // libexec is the standard, lib/crow is for Arch-style distros,
                // ./crow is for the target directory in development builds.
                let possible_locations =
                    ["../libexec/crow", "../lib/crow/crow", "./crow"];
                possible_locations
                    .iter()
    ...
    impl Detect {
        pub fn detect(path: Option<&Path>) -> anyhow::Result<impl InstalledApp> {
            let path = if let Some(path) = path {
                ...
                // ../crow.exe is the standard, lib/crow is for MSYS2, ./crow.exe is for the target
                // directory in development builds.
                let possible_locations =
                    ["../crow.exe", "../lib/crow/crow.exe", "./crow.exe"];
                possible_locations
                    .iter()
                    .find_map(|p| dir.join(p).canonicalize().ok().filter(|path| path != &cli))
                    .context(format!(
